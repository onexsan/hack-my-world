...
label end:
    ...
    "То, что никогда не станет плодом, // вянет там, где нет света // и нет смысла ему находиться. // Там найдешь свой секрет."
    v "(про себя) Секрет? Надо попробовать поискать его..."
